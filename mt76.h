...
#define __mt76_poll_msec(...) \
	____mt76_poll_msec(__VA_ARGS__, 10)

#define mt76_poll_msec(dev, ...) \
	____mt76_poll_msec(&((dev)->mt76), __VA_ARGS__, 10)

#define mt76_poll_msec_tick(dev, ...) \
	____mt76_poll_msec(&((dev)->mt76), __VA_ARGS__)
...
static inline void mt76_packet_id_flush(struct mt76_dev *dev, struct mt76_wcid *wcid)
{
	struct sk_buff_head list;
