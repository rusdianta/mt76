...
static int mt7603_poll_tx(struct napi_struct *napi, int budget)
{
	...
	for (i = MT_TXQ_PSD; i >= 0; i--)
		mt76_queue_tx_cleanup(dev, dev->mt76.q_tx[i].q, false);
...
int mt7603_dma_init(struct mt7603_dev *dev)
{
	...
	ret = mt76_init_tx_queue(&dev->mt76, MT_TXQ_PSD, MT_TX_HW_QUEUE_MGMT,
	...
	ret = mt76_init_tx_queue(&dev->mt76, MT_TXQ_BEACON, MT_TX_HW_QUEUE_BCN,
                 MT_MCU_RING_SIZE, MT_TX_RING_BASE);
	if (ret)
		return ret;

	ret = mt76_init_tx_queue(&dev->mt76, MT_TXQ_CAB, MT_TX_HW_QUEUE_BMC,
                 MT_MCU_RING_SIZE, MT_TX_RING_BASE);
	if (ret)
		return ret;
