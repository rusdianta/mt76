...
void
mt7603_wtbl_set_ps(struct mt7603_dev *dev, struct mt7603_sta *sta,
		   bool enabled)
{
...
void
mt7603_wtbl_sta_ps(struct mt7603_dev *dev, struct mt7603_sta *sta, bool ps)
{
	spin_lock_bh(&dev->ps_lock);
	sta->ps_sleeping = ps;
...
void
mt7603_wtbl_restore_ps(struct mt7603_dev *dev, struct mt7603_sta *sta)
{
	spin_lock_bh(&dev->ps_lock);
	/*
	 * Frames that are already queued for the station belong to the
	 * service period that has just been served, so unlike on a sleep
	 * transition they must not be pulled back into the PS queue.
	 */
	if (sta->ps_sleeping)
...
static void mt7603_mac_watchdog_reset(struct mt7603_dev *dev)
{
	...
	for (i = 0; i < ARRAY_SIZE(dev->mt76.q_tx); i++)
		mt76_queue_tx_cleanup(dev, dev->mt76.q_tx[i].q, true);
...
/*
 * Releasing buffered frames turns off the PSE redirect for a station,
 * since the released frames would otherwise be looped back into the
 * driver PS queue again. mac80211 never tells us when the service period
 * is over, so hardware buffering has to be re-armed here for every
 * station that is still known to be asleep.
 *
 * Waiting for the PSD queue to drain makes sure that the released
